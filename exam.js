...
$('nextBtn').onclick=()=>{if(current<secEnd()){current++;render();return}if(current===95)confirmFinish();else openSectionEndModal()};
const __goNext=goNextSection;goNextSection=function(auto=false){if(auto){sectionIndex++;if(sectionIndex>=4){finish();return}current=sectionIndex*24;sectionSeconds=1500;warned5=false;warned1=false;render();persistSession();showTimeWarning(`بدأ القسم ${sectionIndex+1} من 4 — أمامك 25 دقيقة`);return}openSectionEndModal()};
