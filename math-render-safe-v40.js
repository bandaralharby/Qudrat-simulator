// Qudrat v50 — robust Arabic math + working geometry/chart tokens.
(function(){
 ...
 function render(raw){
  ...
  const visuals=[];
  let protectedRaw=original;
  protectedRaw=protectedRaw.replace(/\{\{shape:(triangle|rect|rectangle|circle|square)(?::([^}]+))?\}\}/g,(_,t,a)=>{const key='QVISUALTOKEN'+visuals.length+'X';visuals.push(shape(t==='rectangle'?'rect':t,a?a.split(':'):[]));return key});
  protectedRaw=protectedRaw.replace(/\{\{chart:bar:([^}]+)\}\}/g,(_,x)=>{const key='QVISUALTOKEN'+visuals.length+'X';visuals.push(bars(x));return key});
  let s=esc(protectedRaw);
  ...
  s=ar(s);
  visuals.forEach((html,i)=>{s=s.replace('QVISUALTOKEN'+i+'X',html)});
  if(!visuals.length){const inferred=inferShape(original);if(inferred)s=inferred+s;}
  return s;
 }
